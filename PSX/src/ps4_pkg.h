...
class PS4PKGWriter {
public:
  PS4PKGWriter() : m_totalDataSize(0) {}
  ...
  HRESULT addItem(const std::string& path, UInt64 size, IInStream* stream,
    bool isFolder, UInt32 flags, UInt32 type) {
    FileEntry entry;
    entry.path = path;
    entry.size = size;
    entry.stream = stream;
  ...
  HRESULT addItemWithBuffer(const std::string& path, UInt64 size,
    std::vector<Byte>&& buffer, bool isFolder,
    ...
    return addItem(path, size, limitedStream, isFolder, flags, type);
  }

  HRESULT writeFPKG(ISequentialOutStream* outStream)
  {
...
private:
  ...
  std::vector<uint8_t> CalculateSHA256(const uint8_t* data, size_t length) {
    ...
    if (CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
      if (CryptCreateHash(hProv, CALG_SHA_256, 0, 0, &hHash)) {
        if (CryptHashData(hHash, data, (DWORD)length, 0)) {
          DWORD hashLen = 32;
  ...
  uint32_t CalculateNameTableSize() {
  
  }

  void BuildNameTable(std::vector<uint8_t>& nameTable) {
